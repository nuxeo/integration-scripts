...
if [ -z "$slaveup" ]; then
    docker run --cpu-period=100000 --cpu-quota=200000 --restart=always --privileged -d --add-host mavenin.nuxeo.com:176.31.239.50 -v /var/run/docker.sock:/var/run/docker.sock:rw -v /opt/jenkins/workspace:/opt/jenkins/workspace:rw -h itslavepriv01 --name=itslavepriv01 -p 3401:22 -t dockerpriv.nuxeo.com:443/nuxeo/jenkins-itpriv
fi
exit 0
